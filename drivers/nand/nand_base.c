	...
	u_char *oob_buf,  struct nand_oobinfo *oobsel, int cached)
{
	...
	case NAND_ECC_HW39_512:
		eccbytes = this->eccbytes;
		unsigned char *free_p , free_buf[512];
		free_p = free_buf; 
		memset(free_p, 0x0, 512);
			     ...
			     struct nand_oobinfo *oobsel)
{
	...
	while (read < len) {
		...
		} else if (NAND_ECC_HW39_512 == eccmode) {
			memset(ecc_code,0xff,eccbytes * this->eccsteps);
			memcpy(ecc_code,&data_poi[end + CFG_NAND_ECC_POS],mtd->oobsize - CFG_NAND_ECC_POS);
			memcpy(&ecc_code[mtd->oobsize - CFG_NAND_ECC_POS],free_pos,eccbytes * this->eccsteps - mtd->oobsize + CFG_NAND_ECC_POS);
			for (i = 0, j = 0, datidx = 0; i < this->eccsteps; i++, datidx += ecc) {
				this->enable_hwecc(mtd,NAND_ECC_READ);
				ecc_status = this->correct_data(mtd, &data_poi[datidx], &ecc_code[j],NULL);
				j += eccbytes;
...
int nand_scan (struct mtd_info *mtd, int maxchips)
{
	...
	for (i = 0; nand_flash_ids[i].name != NULL; i++) {
		...
		if (!nand_flash_ids[i].pagesize) {
			...
			/* The 4th id byte is the important one */
			extid = this->read_byte(mtd);
			/*To support SAMSUNG K9GAG08U0D K9LBG08U0D*/
			if(((master_id == 0xecd5) && (extid_3th == 0x94)) || 
			   ((master_id == 0xecd7) && (extid_3th == 0xd5))) {
				/* Calc pagesize */
				mtd->oobblock = 2048 << (extid & 0x3);
				extid >>= 2;
